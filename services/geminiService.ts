...
import { UserProfile, WeeklyPlan, Recipe, UserAnswer, ChatMessage } from "../types";
...
export const generateWeeklyPlan = async (answers: UserAnswer[]): Promise<WeeklyPlan> => {
...
export const chatWithChef = async (
  ...
  newMessage: string,
  image: string | null,
  mode: 'chef_brain' | 'web_search',
  userProfile?: UserProfile
): Promise<{ text: string; recipe?: Recipe; groundingUrls?: string[] }> => {
 ...
 */
export const generateMealImage = async (recipeDescription: string, size: '1K' | '2K' | '4K'): Promise<string> => {
 ...
 */
export const generateFinalRecipeCard = async (chatContext: string): Promise<Recipe> => {
    const response = await ai.models.generateContent({
        model: 'gemini-3-pro-preview',
