...
import { UserProfile, ChatMessage, Recipe } from '../types';
import { chatWithChef, generateMealImage, generateFinalRecipeCard } from '../services/geminiService';
...
const QuickMeal: React.FC<QuickMealProps> = ({ profile, onSaveRecipe, initialInput, onClearInitialInput }) => {
  const [messages, setMessages] = useState<ChatMessage[]>([]);
  const [input, setInput] = useState(initialInput || "");
  const [isLoading, setIsLoading] = useState(false);
  const [mode, setMode] = useState<'chef_brain' | 'web_search'>('chef_brain');
